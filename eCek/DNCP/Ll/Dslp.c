...
Uint16 Dslp_CRC(Byte* pBuf, int nLen)
{
    int i = 0;
    char j = 0;
...
unsigned short Dslp_GetCRCData(unsigned char *buffer,unsigned len)
{
...
void Dslp_OnReceived(Dslp* self, Byte* data, Uint16 len, Uint8 sourceAddr)
{
    ...
    if (len >= 4)
    {
        ...
        chksum <<= 8;
        chksum |= *p;
        if (chksum == Dslp_GetCRCData(data, len-2))
        {
            fcw.data = data[1];
        ...
        Printf("\n rec %x , CRC %x", chksum, Dslp_GetCRCData(data, len-2));
    }
    else
    {
